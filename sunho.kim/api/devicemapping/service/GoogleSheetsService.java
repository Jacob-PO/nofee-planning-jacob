...
public class GoogleSheetsService {
    ...
    @Value("${google.sheets.sheet-name:summary-mapping}")
    ...
    public void saveMappings(List<DeviceMapping> mappings) {
        if (sheetsService == null || spreadsheetId.isEmpty()) {
            log.warn("⚠️ Google Sheets 미설정 - 저장 건너뜀");
            return;
        }
        ...
        try {
            ...
            log.info("✅ {}개 매핑 저장 완료", mappings.size());
    ...
    public List<DeviceMapping> loadMappings() {
        if (sheetsService == null || spreadsheetId.isEmpty()) {
            log.warn("⚠️ Google Sheets 미설정 - 빈 목록 반환");
